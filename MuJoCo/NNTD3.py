...
class NNTD3(object):    
    ...
    def replay_td(self):
        ...
        for _ in range(int(min(self.config.td_batch_size, len(self.replay_buffer_td.state)) / self.config.mini_batch)):
            ...
            critic_loss = self.a2 * F.mse_loss(target_Q - current_Q, td_error.detach())
    ...
    def train(self, episode_timesteps, batch_size, current_episode):
        ...
        for it in range(episode_timesteps):
            if self.a2 == self.config.eps:
                ...
                for _ in range(int(batch_size / mini_batch)):
                    state, action, next_state, reward, not_done = self.replay_buffer.sample(mini_batch, True)
                    ...
                    td_errors = torch.FloatTensor(np.vstack(td_errors).reshape(-1, 1)).to(self.device)

                    actor_loss = - (self.a1 * self.critic.Q1(state, self.actor(state)) + self.a2 * (td_errors.detach() * nn_loss)).mean()
                    ...
                    critic_loss = F.mse_loss(current_Q1, target_Q) + F.mse_loss(current_Q2, target_Q) + self.config.nn_param_critic * F.mse_loss(target_Q - current_Q, td_errors.detach())
